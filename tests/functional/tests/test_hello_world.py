...
async def test_hello_world_with_openrouter(
    ...
    async with run_bot_server(bot):
        ...
        assert nodes_response.status_code == 200, f"Bot not found in registry: {nodes_response.text}"

        node_data = nodes_response.json()
        assert node_data["id"] == bot.node_id
        assert "ask_math_question" in [r["id"] for r in node_data.get("bots", [])]

        print(f"Bot registered successfully: {bot.node_id}")

        # ====================================================================
        # Step 5: Execute bot through control plane
        # ====================================================================
        execution_request = {
            "input": {
        ...
        execution_response = await async_http_client.post(
            f"/api/v1/bots/{bot.node_id}.ask_math_question",
        ...
        # Validate node ID
        assert result_data["node_id"] == bot.node_id
