...
import { BotBadge, AgentColorDot } from "./BotBadge";
...
export function BotLegend({
  ...
  const workflowAgents = useMemo(() => {
    console.log('BotLegend: Processing nodes:', nodes.length, nodes);
    const agentSet = new Set<string>();

    nodes.forEach((node) => {
      const nodeData = node.data as unknown as WorkflowDAGNode;
      const agentName = nodeData.agent_name || nodeData.node_id;
      console.log('BotLegend: Node agent:', agentName, nodeData);
      if (agentName) {
    ...
    console.log('BotLegend: Extracted agents:', agents);
    return agents;
  }, [nodes]);
  ...
  const agentColors = useMemo(() => {
    ...
    if (workflowAgents.length > 0) {
      botColorManager.cleanupUnusedAgents(workflowAgents);
    }

    return workflowAgents.map((agentName) => {
      // Ensure the agent color is registered
      const agentColor = botColorManager.getAgentColor(agentName);
      return agentColor;
    });
  }, [workflowAgents]);
