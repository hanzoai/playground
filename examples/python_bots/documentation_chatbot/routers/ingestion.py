"""Ingestion bots for the documentation chatbot."""
...
ingestion_router = BotRouter(tags=["ingestion"])
...
async def _clear_namespace_via_api(namespace: str) -> dict:
    ...
    base_url = os.getenv("CONTROL_PLANE_URL") or os.getenv("PLAYGROUND_URL") or ""
    base_url = base_url.rstrip("/")
    if not base_url:
        raise ValueError("CONTROL_PLANE_URL (or PLAYGROUND_URL) is required to clear namespace")

    api_key = os.getenv("CONTROL_PLANE_API_KEY") or os.getenv("PLAYGROUND_API_KEY")
