...
planning_router = BotRouter(prefix="planning")
...
async def create_research_plan(
    ...
    This bot recursively decomposes a research question into smaller,
    ...
    initial_response = await planning_router.ai(
        system=(
            ...
            "   - Example: 'What is Playground?' or 'What are Playground's features?'\n\n"
            "2. **Parent tasks** (have dependencies) = Questions answered by synthesizing children\n"
            "   - These combine answers from child tasks\n"
            "   - Example: 'Summarize Playground' depends on 'What is Playground?' and 'What are features?'\n\n"
    ...
    async def refine_recursively(
        ...
        """Recursively refine a task by calling the refine_task bot."""
        if task.depth >= max_depth:
            # Base case: task is specific enough
            return [task]

        # Directly call the refine_task bot - this creates a workflow node
...
async def refine_task(
    ...
    This is a helper bot that can be called recursively to break down
