...
curl -s "$AGENTS_SERVER/api/v1/mcp/status" | jq . 2>/dev/null || echo "❌ Failed to get MCP status"

# Get first node ID
echo ""
echo "3. Getting available nodes..."
FIRST_NODE=$(curl -s "$AGENTS_SERVER/api/v1/nodes" | jq -r '.[0].id // empty' 2>/dev/null)
...
if [ -n "$FIRST_NODE" ] && [ "$FIRST_NODE" != "null" ]; then
    ...
    curl -s "$AGENTS_SERVER/api/v1/nodes/$FIRST_NODE/mcp/health" | jq . 2>/dev/null || echo "❌ Failed to get node MCP health"

    # Test developer mode
    echo ""
    echo "5. Testing developer mode..."
    curl -s "$AGENTS_SERVER/api/v1/nodes/$FIRST_NODE/mcp/health?mode=developer" | jq . 2>/dev/null || echo "❌ Failed to get developer mode health"
else
