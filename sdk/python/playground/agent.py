...
from playground.agent_mcp import AgentMCP
from playground.agent_registry import clear_current_agent, set_current_agent
...
class Agent(FastAPI):
    """
    Playground Agent - FastAPI subclass for creating AI agent nodes.

    The Agent class is the core component of the Playground SDK that enables developers to create
    ...
    def __init__(
        ...
        **kwargs,
        ...
        max_calls_env = os.getenv("AGENTS_AGENT_MAX_CONCURRENT_CALLS")
        if max_calls_env:
            try:
                ...
                log_warn(
                    f"Invalid AGENTS_AGENT_MAX_CONCURRENT_CALLS='{max_calls_env}', defaulting to {default_limit}"
                )
        else:
            self._max_concurrent_calls = default_limit
    ...
    async def call(self, target: str, *args, **kwargs) -> dict:
        """
        Initiates a cross-agent call to another reasoner or skill via the Playground execution gateway.
