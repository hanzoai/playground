...
class BotUtils:
    ...
    def detect_input_type(input_data: Any) -> str:
        ...
        if isinstance(input_data, str):
            ...
            if input_data.startswith(("http://", "https://")):
                return "image_url" if BotUtils.is_image_url(input_data) else "url"
    ...
    def create_input_schema_from_mcp_tool(
        ...
        for prop_name, prop_def in properties.items():
            prop_type = BotUtils.map_json_type_to_python(
    ...
    def serialize_result(result: Any) -> Any:
        ...
        try:
            if hasattr(result, "model_dump"):  # Pydantic v2
                ...
                return {
                    k: BotUtils.serialize_result(v)
                    for k, v in result.__dict__.items()
                }
            elif isinstance(result, (list, tuple)):
                return [BotUtils.serialize_result(item) for item in result]
            elif isinstance(result, dict):
                return {k: BotUtils.serialize_result(v) for k, v in result.items()}
            else:
