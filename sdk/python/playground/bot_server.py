...
from playground.logger import log_debug, log_error, log_info, log_success, log_warn
from playground.utils import get_free_port
from fastapi import Request
from fastapi.routing import APIRoute


class BotServer:
    """Server management functionality for Playground Agent"""

    def __init__(self, agent_instance):
        """
        Initialize the BotServer with a reference to the agent instance.
    ...
    def setup_playground_routes(self):
        """Setup standard routes that Playground server expects"""
        ...
        async def shutdown_bot(request: Request):
            ...
            This endpoint allows the Playground server to request a graceful shutdown
            ...
            try:
                ...
                # Set shutdown status
                from playground.bot import BotStatus

                self.agent._shutdown_requested = True
                self.agent._current_status = BotStatus.OFFLINE

                # Notify Playground server of shutdown initiation
                try:
        ...
        async def mcp_health():
            """Get MCP health information in the format expected by Playground server"""
            if not self.agent.mcp_manager:
        ...
        async def restart_mcp_server_alt(alias: str):
            """Alternative restart endpoint for Playground server compatibility"""
    ...
    async def _graceful_shutdown(self, timeout_seconds: int = 30):
        ...
        try:
            ...
            try:
                from playground.bot_registry import clear_current_bot
    ...
    def serve(
        ...
        **kwargs,
        ...
        This method implements smart port resolution that seamlessly works with Playground CLI
        or standalone execution. The port selection priority is:
        1. Explicit port parameter (highest priority)
        2. PORT environment variable (Playground CLI integration)
        ...
        if port is None:
            # Check for Playground CLI integration via environment variable
            env_port = os.getenv("PORT")
            if env_port and env_port.isdigit():
                suggested_port = int(env_port)
                if BotUtils.is_port_available(suggested_port):
                    port = suggested_port
                    if self.agent.dev_mode:
                        log_debug(f"Using port from Playground CLI: {port}")
                else:
                    # Playground CLI suggested port is taken, find next available
                    try:
                        port = get_free_port(start_port=suggested_port)
                        if self.agent.dev_mode:
                            log_debug(
                                f"Playground CLI port {suggested_port} taken, using {port}"
                            )
                    except RuntimeError:
                        port = get_free_port()  # Fallback to default range
                        if self.agent.dev_mode:
                            log_debug(f"Using fallback port: {port}")
            elif auto_port or os.getenv("AGENTS_AUTO_PORT") == "true":
                ...
                if BotUtils.is_port_available(8001):
                    port = 8001
                else:
                    try:
                        port = get_free_port()
            ...
            if not BotUtils.is_port_available(port):
                if self.agent.dev_mode:
        ...
        if not self.agent.base_url:
            # Check HANZO_CALLBACK_URL environment variable before defaulting to localhost
            ...
            if env_callback_url:
                ...
                try:
                    ...
                    if parsed.hostname:
                        ...
                        if self.agent.dev_mode:
                            log_debug(
                                f"Using HANZO_CALLBACK_URL from environment: {self.agent.base_url}"
                            )
                    else:
        ...
        async def startup_resilient_lifecycle():
            """Resilient lifecycle startup: connection manager handles Playground server connectivity"""
            ...
            config = ConnectionConfig(
                retry_interval=10.0,  # Check every 10 seconds for Playground server
            ...
            def on_connected():
                if self.agent.dev_mode:
                    log_info(
                        "Connected to Playground server - full functionality available"
                    )
            ...
            def on_disconnected():
                if self.agent.dev_mode:
                    log_warn("Playground server disconnected - running in local mode")
            ...
            # These work independently of Playground server connection
            ...
            if connected:
                if self.agent.dev_mode:
                    log_info("Agent started with Playground server connection")
            else:
                if self.agent.dev_mode:
                    log_info(
                        "Agent started in local mode - will connect to Playground server when available"
                    )
        ...
        async def shutdown_cleanup():
            ...
            if getattr(self.agent, "client", None):
                try:
                    ...
                    if self.agent.dev_mode:
                        log_error(f"Playground client shutdown error: {e}")
