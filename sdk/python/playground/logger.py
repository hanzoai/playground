"""
Playground SDK Logging Utility

This module provides a centralized logging system for the Playground SDK that:
...
class PlaygroundLogger:
    """
    Centralized logger for Playground SDK with configurable verbosity and payload truncation.
    ...
    def __init__(self, name: str = "playground"):
        ...
        self.log_level = os.getenv("AGENTS_LOG_LEVEL", "WARNING").upper()
        self.truncate_length = int(os.getenv("AGENTS_LOG_TRUNCATE", "200"))
        self.show_payloads = (
            os.getenv("AGENTS_LOG_PAYLOADS", "false").lower() == "true"
        )
        self.show_tracking = (
            os.getenv("AGENTS_LOG_TRACKING", "false").lower() == "true"
        )
        self.show_fire = os.getenv("AGENTS_LOG_FIRE", "false").lower() == "true"
    ...
    def _format_payload(self, payload: Any) -> str:
        ...
        if not self.show_payloads:
            return "[payload hidden - set AGENTS_LOG_PAYLOADS=true to show]"
    ...
    def track(self, message: str, **kwargs):
        """Log tracking messages (controlled by AGENTS_LOG_TRACKING)"""
        if self.show_tracking:
            self.logger.debug(f"🔍 TRACK: {self._truncate_message(message)}")

    def fire(self, message: str, payload: Optional[Any] = None, **kwargs):
        """Log fire-and-forget workflow messages (controlled by AGENTS_LOG_FIRE)"""
        if self.show_fire:
...
def get_logger(name: str = "playground") -> PlaygroundLogger:
    """Get or create a Playground SDK logger instance"""
    global _global_logger
    if _global_logger is None:
        _global_logger = PlaygroundLogger(name)
