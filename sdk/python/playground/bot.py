...
from playground.bot_mcp import BotMCP
from playground.bot_registry import clear_current_bot, set_current_bot
...
from playground.did_manager import DIDManager
from playground.vc_generator import VCGenerator
from playground.mcp_client import MCPClientRegistry
from playground.mcp_manager import MCPManager
from playground.memory import MemoryClient, MemoryInterface
from playground.memory_events import MemoryEventClient
from playground.logger import log_debug, log_error, log_info, log_warn
from playground.router import BotRouter
from playground.connection_manager import ConnectionManager
...
from playground.multimodal_response import MultimodalResponse
from playground.async_config import AsyncConfig
from playground.async_execution_manager import AsyncExecutionManager
from playground.pydantic_utils import convert_function_args, should_convert_args
...
class BotEntry:
    """Minimal bot metadata - uses __slots__ for memory efficiency.
...
def _build_callback_candidates(
    ...
    env_callback_url = os.getenv("HANZO_CALLBACK_URL", os.getenv("AGENT_CALLBACK_URL"))
...
def _resolve_callback_url(callback_url: Optional[str], port: int) -> str:
    ...
    2. HANZO_CALLBACK_URL environment variable
...
class Bot(FastAPI):
    """
    Playground Bot - FastAPI subclass for creating AI agent nodes.

    The Bot class is the core component of the Playground SDK that enables developers to create
    intelligent agent nodes. It inherits from FastAPI to provide HTTP endpoints and integrates
    with the Playground ecosystem for distributed AI workflows.

    Key Features:
    - Decorator-based bot and skill registration
    - Cross-agent communication via the Playground execution gateway
    ...
    Example:
        ...
        # Define a bot (AI-powered function)
        @app.bot()
    ...
    def __init__(
        self,
        node_id: str,
        agents_server: str = "http://localhost:8080",
        version: str = "1.0.0",
        ...
        **kwargs,
        ...
        """
        Initialize a new Playground Agent instance.

        Sets log level to DEBUG if dev_mode is True, else INFO.
        """
        # Set logging level based on dev_mode
        from playground.logger import set_log_level

        set_log_level("DEBUG" if dev_mode else "INFO")

        """
        Creates a new agent node that can host bots (AI-powered functions) and skills
        (deterministic functions) while integrating with the Playground ecosystem for distributed
        ...
        Args:
            node_id (str): Unique identifier for this agent node. Used for routing and
                          ...
                          within your Playground ecosystem.
            agents_server (str, optional): URL of the Playground server for registration and
            ...
            callback_url (str, optional): Explicit callback URL for Playground server to reach this agent.
                                         If not provided, will use HANZO_CALLBACK_URL environment variable,
            ...
            vc_enabled (bool | None, optional): Controls default VC generation policy for this agent node.
                                         True enables VCs for all bots/skills (default), False disables,
                                         and None defers entirely to platform defaults.
            api_key (str, optional): API key for authenticating with the Playground control plane.
        ...
        Example:
            ```python
            # Basic agent setup
            app = Bot(node_id="sentiment_analyzer")

            # Advanced configuration
            app = Bot(
                node_id="advanced_agent",
                agents_server="https://playground.company.com",
                version="2.1.0",
                ai_config=AIConfig(
                    provider="openai",
                    model="gpt-4",
        ...
        self.client = PlaygroundClient(
            base_url=agents_server, async_config=self.async_config, api_key=api_key
        )
        ...
        self._current_status: BotStatus = BotStatus.STARTING
        ...
        # Add connection management for resilient Playground server connectivity
        ...
        self._ai_handler: Optional[BotAI] = None
        self._cli_handler: Optional[BotCLI] = None
        # Eager handlers - required for core agent functionality
        self.mcp_handler = BotMCP(self)
        self.agents_handler = PlaygroundHandler(self)
        self.workflow_handler = BotWorkflow(self)
        self.server_handler = BotServer(self)
        ...
        if self._enable_mcp:
            try:
                agent_dir = self.mcp_handler._detect_bot_directory()
        ...
        # Setup standard Playground routes and memory event listeners
        self.server_handler.setup_playground_routes()
        ...
        max_calls_env = os.getenv("PLAYGROUND_BOT_MAX_CONCURRENT_CALLS", os.getenv("PLAYGROUND_BOT_MAX_CONCURRENT_CALLS"))
        if max_calls_env:
            try:
                ...
                log_warn(
                    f"Invalid PLAYGROUND_BOT_MAX_CONCURRENT_CALLS='{max_calls_env}', defaulting to {default_limit}"
                )
        else:
            self._max_concurrent_calls = default_limit
    ...
    def bots(self) -> List[Dict]:
        ...
        """
        result = []
        for entry in self._bot_registry.values():
            result.append(self._entry_to_metadata(entry, "bot"))
        return result

    @bots.setter
    def bots(self, value: List[Dict]) -> None:
        """Allow setting bots for backward compatibility (deprecated)."""
        self._bots_legacy = value
    ...
    def _entry_to_metadata(self, entry: Union[BotEntry, SkillEntry], kind: str) -> Dict:
    ...
    def handle_serverless(self, event: dict, adapter: Optional[Callable] = None) -> dict:
        """
        Universal serverless handler for executing bots and skills.
        ...
        Special Endpoints:
            - /discover: Returns agent metadata for Playground server registration
            - /execute: Executes bots and skills
        ...
        Args:
            event (dict): Serverless event containing:
                ...
                - bot: Name of the bot to execute (for execution)
        ...
        Example:
            ...
            app = Bot("my_agent", auto_register=False)
        ...
        if path == "/discover" or path.endswith("/discover") or action == "discover":
            # Return agent metadata for Playground server registration
        ...
        if self.auto_register and not self.agents_connected:
            try:
                ...
                self.agents_handler._register_bot()
        ...
        bot_name = (
            event.get("bot") or event.get("target") or event.get("skill")
        )
        if not bot_name and path:
            # Support paths like /execute/<target> or /bots/<name>
            cleaned_path = path.split("?", 1)[0].strip("/")
            parts = cleaned_path.split("/")
            if parts and parts[0] not in ("", "discover"):
                if len(parts) >= 2 and parts[0] in ("execute", "bots", "skills"):
                    bot_name = parts[1]
                elif parts[0] in ("execute", "bots", "skills"):
                    bot_name = None
                elif parts:
                    bot_name = parts[-1]
        ...
        if not bot_name:
            return {
                "statusCode": 400,
                "body": {"error": "Missing 'bot' or 'target' in event"},
            }
        ...
        try:
            ...
            if hasattr(self, bot_name):
                func = getattr(self, bot_name)
                ...
                return {
                    "statusCode": 404,
                    "body": {"error": f"Function '{bot_name}' not found"},
                }
    ...
    def _handle_discovery(self) -> dict:
        ...
        Returns agent metadata including bots, skills, and configuration
        for automatic registration with the Playground server.
    ...
    def memory(self) -> Optional[MemoryInterface]:
        ...
        Returns:
            ...
            None: If no execution context is available (e.g., outside of bot/skill execution).
    ...
    def ctx(self) -> Optional[ExecutionContext]:
        ...
        Example:
            ...
            ```
        """
        # Check thread-local context first (set during active bot/skill execution)
    ...
    def _set_bot_vc_override(
        self, bot_id: str, value: Optional[bool]
    ) -> None:
        if value is None:
            self._bot_vc_overrides.pop(bot_id, None)
        else:
            self._bot_vc_overrides[bot_id] = value
    ...
    def _build_bot_metadata(self) -> Optional[Dict[str, Any]]:
    ...
    def _build_vc_metadata(self) -> Dict[str, Any]:
        ...
        effective_bots = {
            bot["id"]: self._effective_component_vc_setting(
                bot["id"], self._bot_vc_overrides
            )
        ...
        return {
            "agent_default": self._bot_vc_default(),
            "bot_overrides": dict(self._bot_vc_overrides),
    ...
    def _apply_discovery_response(self, payload: Optional[Dict[str, Any]]) -> None:
        """Update agent networking state from Playground discovery response."""
        ...
        if resolved and resolved != self.base_url:
            log_debug(f"Applying resolved callback URL from Playground: {resolved}")
    ...
    def _register_bot_with_did(self) -> bool:
        """
        ...
        try:
            # Prepare bot and skill definitions for DID registration
            bot_defs = []
            for bot in self.bots:
                bot_defs.append(
                    {
                        "id": bot["id"],
                        "input_schema": bot["input_schema"],
            ...
            log_debug(
                "Calling did_manager.register_bot() with "
                f"{len(bot_defs)} bots and {len(skill_defs)} skills"
            )

            # Register with DID system
            success = self.did_manager.register_bot(bot_defs, skill_defs)
            if success:
                ...
                if self.dev_mode:
                    ...
                    log_info(f"DID: {self.did_manager.get_bot_did()}")
            else:
                if self.dev_mode:
    ...
    def _setup_playground_routes(self):
        """Delegate to server handler for route setup"""
        return self.server_handler.setup_playground_routes()
    ...
    def bot(
        ...
        *,
        ...
        """
        Decorator to register a bot function.

        A bot is an AI-powered function that takes input and produces structured output using LLMs.
        It automatically handles input/output schema generation and integrates with the Playground's AI capabilities.

        Args:
            path (str, optional): The API endpoint path for this bot. Defaults to /bots/{function_name}.
            name (str, optional): Explicit Playground registration ID. Defaults to the function name.
            tags (List[str] | None, optional): Organizational tags that travel with the bot metadata.
            vc_enabled (bool | None, optional): Override VC generation for this bot. True forces VC creation,
        ...
        def decorator(func: Callable) -> Callable:
            ...
            bot_id = decorator_name or func_name
            endpoint_path = decorator_path or f"/bots/{func_name}"
            ...
            self._set_bot_vc_override(bot_id, vc_enabled)
            ...
            async def endpoint(request: Request):
                ...
                async def run_bot() -> Any:
                    return await self._execute_bot_endpoint(
                        bot_id=bot_id,
                        func=func,
                        signature=sig,
            ...
            async def tracked_func(*args, **kwargs):
                ...
                # 🔥 CRITICAL FIX: Always use workflow tracking for direct bot calls
            ...
            if decorator_tags:
                ...
                decorator_tag_attr = getattr(original_func, "_bot_tags", None)
            ...
            # NOTE: Legacy storage removed - bots property generates list on-demand
            # self.bots.append(bot_metadata)  # REMOVED - use _bot_registry
            # self._bot_return_types[bot_id] = return_type  # REMOVED - stored in entry
            ...
            if bot_id != func_name:
                setattr(self, bot_id, getattr(self, func_name, tracked_func))
    ...
    async def _execute_bot_endpoint(
        self,
        *,
        bot_id: str,
        func: Callable,
        ...
        if hasattr(self, "workflow_handler") and self.workflow_handler:
            execution_context.bot_name = bot_id
        ...
        try:
            try:
                ...
                raise ValidationError(
                    f"Pydantic validation failed for bot '{bot_id}': {exc}",
                    model=getattr(exc, "model", None),
                ) from exc
            except Exception as exc:  # pragma: no cover - best effort log
                if self.dev_mode:
                    log_debug(
                        f"⚠️ Warning: Failed to convert arguments for {bot_id}: {exc}"
                    )
                args, kwargs = (), payload_dict
    ...
    async def _execute_async_with_callback(
        self,
        *,
        bot_coro: Callable[[], Awaitable[Any]],
        execution_id: str,
        bot_name: str,
    ) -> None:
        if not execution_id:
    ...
    async def _post_execution_status(
        ...
        if not self.client:
            log_error("Playground client unavailable; cannot send status updates")
            return
    ...
    def skill(
        ...
        *,
        ...
        - Enables cross-agent communication via the Playground execution gateway
        ...
        Returns:
            Callable: The decorated function with enhanced Playground integration.
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def endpoint(request: Request):
                ...
                if handler:
                    execution_context.bot_name = skill_id
            ...
            async def _run_async_skill(*args, **kwargs):
                ...
                child_context.bot_name = skill_id
    ...
    def include_router(
        ...
        """Augment FastAPI's include_router to understand BotRouter."""

        if isinstance(router, BotRouter):
            router._attach_bot(self)
            ...
            def _normalize_component_path(
                ...
                """Ensure router-registered components map to /bots/{id} style paths."""
            ...
            for entry in router.bots:
                if entry.get("registered"):
                    continue

                func = entry["func"]
                default_path = f"/bots/{func.__name__}"
                ...
                explicit_bot_name = entry_kwargs.pop("name", None)
                bot_id = explicit_bot_name or _build_prefixed_name(
                    namespace_segments,
                    func.__name__,
                )
                ...
                # This enables direct bot-to-bot calls to go through tracking
        ...
        **kwargs,
        ...
        Args:
            *args: Flexible inputs - text, images, audio, files, or mixed content.
            ...
            memory_scope (List[str], optional): Memory scopes to inject (e.g., ['workflow', 'session', 'bot']).
    ...
    async def call(self, target: str, *args, **kwargs) -> dict:
        """
        Initiates a cross-agent call to another bot or skill via the Playground execution gateway.

        This method allows agents to seamlessly communicate and utilize bots/skills
        deployed on other agent nodes within the Playground ecosystem. It properly propagates
        ...
        **Return Type**: Always returns JSON/dict objects, similar to calling any REST API.
        ...
        Args:
            target (str): The full target ID in format "node_id.bot_name" or "node_id.skill_name"
                         (e.g., "classification_team.classify_ticket", "support_agent.send_email").
            *args: Positional arguments to pass to the target bot/skill. These will be
                   automatically mapped to the target function's parameter names in order.
            **kwargs: Keyword arguments to pass to the target bot/skill.

        Returns:
            dict: The result from the target bot/skill execution as JSON/dict.
                  Always returns dict objects, like calling any REST API.

        Examples:
            # Bot call - returns dict (convert to Pydantic manually if needed)
            ...
            # Error handling
            try:
                result = await app.call("some_agent.some_bot", data="test")
        ...
        if args:
            ...
            # Try to get parameter names from the target (if it's a local bot/skill)
            if "." in target:
        ...
        if self.dev_mode:
            ...
            from playground.logger import log_debug
        ...
        # DISABLED: Same-agent call detection - Force all calls through Playground server
        # This ensures all app.call() requests go through the Playground server for proper
        ...
        if not self.agents_connected:
            ...
            log_warn(
                f"Playground server unavailable - cannot make cross-agent call to {target}"
            )
            raise Exception(
                f"Cross-agent call to {target} failed: Playground server unavailable. Agent is running in local mode."
            )

        # Use the enhanced PlaygroundClient to make the call via execution gateway
        try:
    ...
    async def _cleanup_async_resources(self) -> None:
        ...
        if getattr(self, "client", None) is not None:
            try:
                ...
                if self.dev_mode:
                    log_debug("PlaygroundClient resources closed")
            except Exception as e:
                if self.dev_mode:
                    log_debug(f"Error closing PlaygroundClient resources: {e}")

    def note(self, message: str, tags: List[str] = None) -> None:
        """
        Add a note to the current execution for debugging and tracking purposes.

        This method sends a note to the Playground server asynchronously without blocking
        the current execution. The note is automatically associated with the current
        execution context and can be viewed in the Playground UI for debugging and monitoring.
    ...
    def _get_target_return_type(self, target: str) -> Optional[Type]:
        """
        Get the return type for a target bot.

        Args:
            target: Target in format 'node_id.bot_name'
        ...
        return_type_map = getattr(self, "_bot_return_types", None)
        if return_type_map:
        ...
        for bot in self.bots:
            if bot.get("id") == function_name:
                stored_type = bot.get("return_type")
    ...
    @classmethod
    def get_current(cls) -> Optional["Bot"]:
        """
    ...
    def discover(
        ...
        if not self.client:
            raise RuntimeError("Playground client is not configured")
    ...
    def serve(  # pragma: no cover - requires full server runtime integration
        ...
        **kwargs,
        ...
        """
        Start the agent node server with intelligent port management and Playground integration.

        This method launches the agent as a FastAPI server that can receive bot and skill
        requests from other agents via the Playground execution gateway. It handles automatic
        registration with the Playground server, heartbeat management, and graceful shutdown.

        The server provides:
        - RESTful endpoints for all registered bots and skills
        - Health check endpoints for monitoring
        - MCP server status and management endpoints
        - Automatic Playground server registration and heartbeat
        ...
        Args:
            ...
            heartbeat_interval (int): The interval in seconds for sending heartbeats to the Playground server.
        ...
        Server Endpoints:
            ...
            - `POST /bots/{bot_name}`: Execute bot functions
        ...
        Integration with Playground:
            - Automatically registers with Playground server on startup
            - Sends periodic heartbeats to maintain connection
            - Receives execution requests via Playground's routing system
        ...
        Lifecycle:
            ...
            3. Playground server registration
            4. Heartbeat loop starts
            5. Ready to receive requests
            6. Graceful shutdown on SIGINT/SIGTERM
            7. MCP server cleanup
            8. Playground server deregistration

        Note:
            - The server runs indefinitely until interrupted (Ctrl+C)
            - All registered bots and skills become available as REST endpoints
...
# Backward-compatible alias so code importing ``from playground.bot import Agent`` continues to work.
Agent = Bot
