...
from playground.logger import log_debug, log_warn
...
class BotWorkflow:
    """Workflow helper that keeps local execution metadata in sync with Playground."""
    ...
    async def execute_with_tracking(
        ...
        bot_name = getattr(original_func, "__name__", "bot")

        parent_context = self._get_parent_context()
        execution_context = self._build_execution_context(bot_name, parent_context)
    ...
    async def fire_and_forget_update(self, payload: Dict[str, Any]) -> None:
        """Send workflow update to Playground when a client is available."""

        client = getattr(self.agent, "client", None)
        base_url = getattr(self.agent, "agents_server", None)
    ...
    def _build_execution_context(
        ...
        if parent_context:
            ...
            context = ExecutionContext.create_new(
                getattr(self.agent, "node_id", "agent"), bot_name
            )
            context.bot_name = bot_name
