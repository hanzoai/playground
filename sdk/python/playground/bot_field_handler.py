...
class PlaygroundHandler:
    """
    Playground Server Communication handler for Agent class.

    This class encapsulates all Playground server communication functionality including:
    - Agent registration with Playground server
    ...
    def __init__(self, agent_instance):
        """
        Initialize the Playground handler with a reference to the agent instance.
    ...
    async def register_with_playground_server(self, port: int):
        """Register this agent node with Playground server"""
        ...
        log_debug(
            f"HANZO_CALLBACK_URL env var: {os.environ.get('HANZO_CALLBACK_URL', os.environ.get('AGENT_CALLBACK_URL', 'NOT_SET'))}"
        )
        log_debug(f"Port: {port}")
        log_debug(f"Running in container: {_is_running_in_container()}")
        log_debug(
            f"All env vars containing 'HANZO' or 'AGENT': {[k for k in os.environ.keys() if 'HANZO' in k.upper() or 'AGENT' in k.upper()]}"
        )
        ...
        try:
            ...
            success, payload = await self.agent.client.register_bot(
            ...
            if success:
                ...
                if self.agent.did_manager:
                    did_success = self.agent._register_bot_with_did()
    ...
    async def notify_shutdown(self) -> bool:
        """
        Notify Playground server of graceful shutdown.
    ...
    def setup_fast_lifecycle_signal_handlers(
        ...
        - Only intercepts SIGTERM to mark the agent offline and notify Playground immediately.
        - Leaves SIGINT (Ctrl+C) to uvicorn so its shutdown hooks run and resources are cleaned up.
        """

        def signal_handler(signum: int, frame) -> None:
            """Handle SIGTERM: mark offline, notify Playground, then re-emit the signal for default handling."""
            ...
            # Best-effort immediate notification to Playground
            try:
    ...
    async def register_with_fast_lifecycle(
        ...
        """
        from playground.bot import _build_callback_candidates, _resolve_callback_url
        ...
        log_debug(
            f"Fast lifecycle - HANZO_CALLBACK_URL env var: {os.environ.get('HANZO_CALLBACK_URL', os.environ.get('AGENT_CALLBACK_URL', 'NOT_SET'))}"
        )
        ...
        try:
            ...
            if success:
                ...
                if self.agent.dev_mode:
                    log_success(
                        f"Fast registration successful - Status: {BotStatus.STARTING.value}"
                    )
                self.agent.agents_connected = True
