"""
CLI functionality for Playground Agent class.
...
from playground.logger import log_error, log_warn
...
class BotCLI:
    ...
    def _get_all_functions(self) -> List[str]:
        """Get list of all available bots and skills"""
        functions = []

        # Add bots
        for bot in self.agent.bots:
            functions.append(bot["id"])
    ...
    def _list_functions(self) -> None:
        ...
        if self.agent.bots:
            print("Bots (AI-powered):")
            for bot in self.agent.bots:
                func = self._get_function(bot["id"])
                if func:
                    sig = inspect.signature(func)
        ...
        print(
            f"Total: {len(self.agent.bots)} bots, {len(self.agent.skills)} skills\n"
        )
    ...
    def _interactive_shell(self) -> None:
        ...
        try:
            ...
            # Add all skills and bots to namespace
            for bot in self.agent.bots:
                func = self._get_function(bot["id"])
                if func:
                    namespace[bot["id"]] = func
