...
def setup_litellm_stub(monkeypatch):
    ...
    monkeypatch.setattr("playground.bot_ai.litellm", module, raising=False)
    return module
...
def test_get_rate_limiter_cached(monkeypatch, agent_with_ai):
    ...
    monkeypatch.setattr("playground.bot_ai.StatelessRateLimiter", DummyLimiter)
...
async def test_ai_simple_text(monkeypatch, agent_with_ai):
    ...
    monkeypatch.setattr("playground.bot_ai.BotUtils.detect_input_type", lambda value: "text")
    monkeypatch.setattr("playground.bot_ai.BotUtils.serialize_result", lambda value: value)
