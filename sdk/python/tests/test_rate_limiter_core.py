...
from playground.rate_limiter import RateLimitError, StatelessRateLimiter
...
async def test_execute_with_retry_eventual_success(monkeypatch):
    ...
    monkeypatch.setattr("playground.rate_limiter.asyncio.sleep", fake_sleep)
...
async def test_circuit_breaker_blocks_and_recovers(monkeypatch):
    ...
    clock = Clock(100.0)
    monkeypatch.setattr("playground.rate_limiter.time.time", clock.time)
