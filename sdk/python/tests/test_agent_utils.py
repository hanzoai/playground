...
def test_detect_input_type_and_helpers(tmp_path):
    # URL detection
    assert BotUtils.detect_input_type("https://example.com/img.png") == "image_url"
    assert BotUtils.detect_input_type("https://example.com/") == "url"
    ...
    assert BotUtils.is_audio_url("a.mp3") is True
    assert BotUtils.get_mime_type(".png") == "image/png"


def test_generate_skill_name_and_schema():
    name = BotUtils.generate_skill_name("srv-1", "tool name!")
    ...
    Model = BotUtils.create_input_schema_from_mcp_tool("skill", tool)
    inst = Model(q="text")
    assert inst.q == "text"
...
def test_detect_input_type_bytes_and_structured(tmp_path):
    ...
    assert BotUtils.detect_input_type(b"RIFFxxxxWAVEfmt ") == "audio_bytes"
    assert BotUtils.detect_input_type(str(audio_path)) == "audio_file"
...
def test_serialize_result_handles_complex_objects():
    ...
    serialized = BotUtils.serialize_result(Wrapper())
...
def test_is_port_available(monkeypatch, available):
    ...
    result = BotUtils.is_port_available(4321)
    assert result is available
    assert calls.get("created") is True


def test_detect_input_type_additional_branches(tmp_path):
    assert BotUtils.detect_input_type("data:image/png;base64,AAA") == "image_base64"
    assert BotUtils.detect_input_type("data:audio/wav;base64,AAA") == "audio_base64"
    ...
    assert BotUtils.detect_input_type(str(doc)) == "document_file"
    assert BotUtils.detect_input_type(str(video)) == "video_file"
    assert BotUtils.detect_input_type(b"%PDF-1.4 more bytes") == "document_bytes"
    assert BotUtils.detect_input_type(12345) == "unknown"


def test_create_input_schema_handles_empty_properties():
    Model = BotUtils.create_input_schema_from_mcp_tool("skill", {"input_schema": {}})
    instance = Model()
...
def test_serialize_result_falls_back_to_string():
    ...
    problem = Problem()
    serialized = BotUtils.serialize_result(problem)
