...
func (b *Bot) runCLI(ctx context.Context, args []string) error {
	if !b.hasCLIBots() {
		return &CLIError{Code: 2, Err: errors.New("no CLI bots registered; add bot.WithCLI() to a bot")}
	}

	inv, err := b.parseCLIArgs(args)
	if err != nil {
		b.printHelp("", inv.useColor)
	...
	case inv.command == "list":
		b.printList(inv.useColor)
		return nil
	case inv.command == "help" || inv.help:
		b.printHelp(inv.helpTarget, inv.useColor)
		return nil
	}

	botName := inv.command
	if botName == "" {
		botName = b.defaultCLIBot
	}
	if botName == "" {
		b.printHelp("", inv.useColor)
		return &CLIError{Code: 2, Err: errors.New("no default CLI bot configured")}
	}

	bot, ok := b.bots[botName]
	if !ok || !bot.CLIEnabled {
		return &CLIError{Code: 2, Err: fmt.Errorf("bot %q is not available for CLI use", botName)}
	}
	...
	result, execErr := b.Execute(ctx, botName, inv.input)

	formatter := bot.CLIFormatter
	if formatter == nil {
...
func (b *Bot) parseCLIArgs(args []string) (cliInvocation, error) {
	inv := cliInvocation{
		setValues:    make(map[string]string),
		useColor:     b.cfg.CLIConfig == nil || !b.cfg.CLIConfig.DisableColors,
		outputFormat: "pretty",
	}
	if cfg := b.cfg.CLIConfig; cfg != nil && strings.TrimSpace(cfg.DefaultOutputFormat) != "" {
...
func (b *Bot) printList(useColor bool) {
	bots := make([]*BotHandler, 0, len(b.bots))
	for _, r := range b.bots {
		if r.CLIEnabled {
			bots = append(bots, r)
		}
	}
	...
	if len(bots) == 0 {
		fmt.Println("No CLI bots registered.")
		return
	}

	fmt.Println(colorText(useColor, ansiBold, "Available bots:"))
	for _, r := range bots {
		label := r.Name
		if r.DefaultCLI || b.defaultCLIBot == r.Name {
			label += " (default)"
		}
...
func (b *Bot) printHelp(botName string, useColor bool) {
	cfg := b.cfg.CLIConfig
	...
	if botName == "" {
		...
		fmt.Println("  serve          Start bot server")
		fmt.Println("  list           List available bots")
		...
		if len(bots) > 0 {
			fmt.Println()
			fmt.Println(colorText(useColor, ansiBold, "Bots:"))
			for _, r := range bots {
				name := r.Name
				if r.DefaultCLI || b.defaultCLIBot == r.Name {
					name += " (default)"
...
func (b *Bot) printVersion() {
	fmt.Printf("Playground SDK: v%s\n", sdkVersion)
	fmt.Printf("Bot: %s v%s\n", b.cfg.NodeID, b.cfg.Version)
	fmt.Printf("Go: %s\n", runtime.Version())
}
