...
func TestRegisterBot(t *testing.T) {
	...
	// Register bot
	agent.RegisterBot("test", func(ctx context.Context, input map[string]any) (any, error) {
		return map[string]any{"result": "ok"}, nil
	})

	// Verify registration
	bot, ok := agent.bots["test"]
	assert.True(t, ok)
	assert.NotNil(t, bot)
	assert.Equal(t, "test", bot.Name)
	assert.NotNil(t, bot.Handler)
}

func TestRegisterBot_WithOptions(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
	...
	bot := agent.bots["test"]
	assert.Equal(t, inputSchema, bot.InputSchema)
	assert.Equal(t, outputSchema, bot.OutputSchema)
}

func TestRegisterBot_NilHandler(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
...
func TestInitialize_NoBots(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
	...
	err = agent.Initialize(context.Background())
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "no bots registered")
}
...
func TestHandleBot_Sync(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
	...
	req, err := http.NewRequest(http.MethodPost, server.URL+"/bots/test", bytes.NewReader(reqBody))
	require.NoError(t, err)
...
func TestHandleBot_NotFound(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
	...
	req, err := http.NewRequest(http.MethodPost, server.URL+"/bots/nonexistent", bytes.NewReader([]byte("{}")))
	require.NoError(t, err)
...
func TestHandleBot_WrongMethod(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
	...
	req, err := http.NewRequest(http.MethodGet, server.URL+"/bots/test", nil)
	require.NoError(t, err)
...
func TestHandleBot_Error(t *testing.T) {
	cfg := Config{
		NodeID:        "node-1",
		Version:       "1.0.0",
		PlaygroundURL: "https://api.example.com",
	...
	req, err := http.NewRequest(http.MethodPost, server.URL+"/bots/test", bytes.NewReader([]byte("{}")))
	require.NoError(t, err)
	req.Header.Set("Content-Type", "application/json")
...
func TestHandleBotAsyncPostsStatus(t *testing.T) {
	...
	agent.RegisterBot("demo", func(ctx context.Context, input map[string]any) (any, error) {
	...
	reqBody := []byte(`{"value":42}`)
	req, err := http.NewRequest(http.MethodPost, server.URL+"/bots/demo", bytes.NewReader(reqBody))
	require.NoError(t, err)
	req.Header.Set("Content-Type", "application/json")
...
func TestChildContext(t *testing.T) {
	...
	child := parent.ChildContext("node-1", "child-bot")
	...
	assert.Equal(t, "node-1", child.NodeID)
	assert.Equal(t, "child-bot", child.BotName)
...
func TestBuildChildContext(t *testing.T) {
	...
	assert.Equal(t, "node-1", child.NodeID)
	assert.Equal(t, "child", child.BotName)
...
func TestBuildChildContextRoot(t *testing.T) {
	...
	child := ag.buildChildContext(ExecutionContext{}, "root-bot")
	...
	assert.Equal(t, "node-1", child.NodeID)
	assert.Equal(t, "root-bot", child.BotName)
}
...
func TestCallLocalEmitsEvents(t *testing.T) {
	...
	ag.RegisterBot("child", func(ctx context.Context, input map[string]any) (any, error) {
	...
	for _, evt := range received {
		assert.Equal(t, "child", evt.BotID)
...
func TestCallLocalUnknownBot(t *testing.T) {
	cfg := Config{
		NodeID:  "node-1",
		Version: "1.0.0",
		Logger:  log.New(io.Discard, "", 0),
