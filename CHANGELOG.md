...
- Feat: wallet withdraw refunds to user balance + usage endpoint

1. WithdrawWallet now deposits withdrawn USD back to user's Commerce
   balance (reverse of fund flow: service account → user).

2. New GET /bots/:botId/wallet/usage endpoint shows LLM token usage
   and costs from the bot service account's Commerce transactions.

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com> (cc60644)
...
- Fix: set cloud bot security mode to 'off' for unrestricted terminal access (151a4b2)

## [0.1.41-rc.331] - 2026-03-30


### Fixed

- Fix: use :executionId param in route to match EnhancedExecutionDetailPage (81c6489)
...
- Fix: add /executions/:id route alias for execution detail page (49ac710)

## [0.1.41-rc.328] - 2026-03-29


### Fixed

- Fix: default target to 'main' for cloud bots without dot separator (21882fc)

## [0.1.41-rc.327] - 2026-03-29
...
- Fix: allow cloud bot execution without dot-separated target format (0b858ea)

## [0.1.41-rc.321] - 2026-03-29


### Fixed

- Fix: use Postgres-compatible DDL for wallet tables, ensure tables on all queries (ec4a354)

## [0.1.41-rc.320] - 2026-03-29


### Fixed

- Fix: auto-create wallet tables and wallet row on fund (51032d8)
...
- Fix: handle missing wallet tables gracefully (return empty, not 500) (0e19eb2)
...
- Feat: wire billing and wallet UI to real backends

Phase 3 of billing integration:
- billingApi: route through /v1/billing proxy instead of direct Commerce
- botWalletApi: flip mock default from always-mock to opt-in mock

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com> (36c1b25)
...
- Fix: resolve undefined apiKey compile error in provisioner

Replace remaining references to the removed apiKey variable with
serviceKey for gateway token fallback.

Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com> (5f333e8)
...
- Fix: use cloud-entrypoint.sh for combined desktop pods (nil Args) (8255cdf)
...
- Fix: shareProcessNamespace + desktop-exec wrapper for GUI control (7c123ad)

## [0.1.41-rc.297] - 2026-03-27


### Fixed

- Fix: share X11 socket between agent and operative for desktop control (61c816a)

## [0.1.41-rc.296] - 2026-03-27


### Fixed

- Fix: remove workingDir from agent container - causes MODULE_NOT_FOUND (a9402cd)

## [0.1.41-rc.295] - 2026-03-27


### Fixed

- Fix: run init-workspace as node user with fsGroup, remove chown (8e32381)

## [0.1.41-rc.294] - 2026-03-27


### Fixed

- Fix: use initContainer + workingDir for agent workspace to prevent ENOENT uv_cwd (9f43fe0)

## [0.1.41-rc.293] - 2026-03-27


### Fixed

- Fix: create workspace dir on agent pod startup to prevent ENOENT (cadb613)
...
- Fix: bot details for cloud nodes without bot defs, fix SQLite datetime for PostgreSQL (1fceb9e)

## [0.1.41-rc.286] - 2026-03-26


### CI

- Ci: restore Docker build cache (4cba32f)

- Ci: disable Docker cache for fresh frontend build (5cd8658)



### Fixed

- Fix: update commerce API URL in .env.production, upgrade @hanzo/iam to 0.4.2 (36ed243)
...
- Fix: normalize botID format and fix commerce API URL in network service (411db0a)

## [0.1.41-rc.283] - 2026-03-26


### Fixed

- Fix: accept plain node ID for bot detail/metrics/executions endpoints (1228a2a)
...
- Fix: resolve TypeScript errors from recharts and React 19.2 upgrades

EarningsChart: handle optional value in recharts Tooltip formatter
auto-expanding-textarea: use React.InputEvent for onInput handler (611a959)

## [0.1.41-rc.273] - 2026-03-21


### Chores

- Chore: update dependencies to latest (0e54cb6)
...
- Test: add server routes test (8095b77)
...
- Fix(tasks): handle empty depends_on in GetNextPendingTask SQL (eaa532a)
...
- Fix(lint): check json.Unmarshal and tx.Rollback returns (813b871)
...
- Fix: prefix unused unreadCount variable (7dbe74d)
...
- Fix: use hanzo-bot IAM client ID (org-app naming convention) (885376b)
...
- Fix(e2e): make sidebar and responsive tests more resilient (8f736b6)
...
- Rebrand: replace Inter with Geist Sans (2d771e8)
...
- Fix: ensure gateway WebSocket connected app-wide via useGateway hook

Adds useGateway() to AppContent so the gateway WS connection is
established on app load rather than only when specific pages mount.
Terminal, chat, and node status all depend on this connection. (0bb733d)
...
- Feat(terminal): state-aware terminal with gateway probe and connection overlay

Terminal now probes the gateway via node.describe RPC before enabling
input. Shows status overlays for provisioning, starting, offline, and
disconnected states with retry support. Properly handles gateway
reconnection and differentiates NOT_CONNECTED from WS disconnection. (99c3c9e)
...
- Feat(nav): convert environment selector to interactive dropdown

Replaces static text with a DropdownMenu supporting Production,
Staging, and Development environments. Selection persists via
tenantStore/localStorage. (30d9335)
...
- Fix(terminal): use actual nodeId instead of hardcoded 'main'

NodeDetailPage terminal was always connecting to agentId='main' regardless
of which node was being viewed. Now uses the route parameter nodeId. (ad9260f)
...
- Update e2e visual regression screenshots (855415d)
...
- Add no-cache headers for HTML responses to prevent stale deploys (dcd254d)
...
- Rebrand: replace litellm dep with hanzo-llm (1a6fe01)
...
- Rename 'Hanzo Playground' to 'Hanzo Bot' on login page and client display name (#37)

Co-authored-by: Claude Opus 4.6 <noreply@anthropic.com> (ed3d6b5)
...
- Fix(build): resolve TypeScript errors in TerminalPanel and tenantStore (b23df68)
...
- Fix: add missing react-hook-form and date-fns dependencies

Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com> (677d74b)
...
- Fix: rename cmd/af to cmd/playground for goreleaser, fix .gitignore (eb122f1)
